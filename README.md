...
демо достаточно поднять сервис:
...
1. Выберите **сценарий** (`p1y` / `p10y` / `p100y`) и **bbox** (центр карты или текущий вид).
2. **Пересчитать** → поверх OSM ляжет полупрозрачный растр (Viridis); справа — агрегаты
   (доля p>0.5, p>0.8, средняя вероятность, площадь).
3. **Экспорт** → ZIP с `prediction.tif` + `aggregates.geojson` + `report.pdf`.
4. Режим **«Объяснение»** → клик по карте → панель важности признаков + слой атрибуции.

> Зона покрытия валидированной модели v1 — окрестности **Тулуна (р. Ия)**, bbox
> `[99.9, 54.05, 101.5, 55.15]` (EPSG:32647). Вне неё работает **глобальный режим** (см. ниже).
...
лучше baseline по **всем** метрикам (IoU 0.0345 vs 0.0212; PR-AUC 0.0353 vs 0.0251;
ROC-AUC 0.890 vs 0.854; Brier 0.067 vs 0.164 — значимо), но 95 %-CI по IoU/F1/PR-AUC
**пересекаются** (всего 32 test-тайла). Лимит — **датасет** (одно событие, сильный дисбаланс,
шум разметки), не кодировка признаков. Путь к pass — расширение датасета (>1 события/региона),
это отдельный R&D, вне Этапа 6.
...
- **OQ-8 (дисперсия метрик между прогонами, порог ±2 % IoU) не измерялась.** Методика замера:
  обучить U-Net ≥ 2 раза с разными сидами, прогнать `make eval`, сравнить разброс IoU/F1.
  Текущие модели обучены одним прогоном.
